...
const LiveIntelligenceDashboard = () => {
  ...
  const getMockData = () => ({
    ...
    metadata: {
      ...
      screaming_frog: { total_urls_crawled: 150 },
      surfer_seo: { word_count: 2450 },
      lighthouse: { speed_index: 2100, time_to_interactive: 3450 }
    }
  });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="animate-spin rounded-full h-16 w-16 border-4 border-slate-700 border-t-emerald-500" />
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={i} className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl">
              <p className="text-slate-400 text-xs uppercase mb-1">{m.label}</p>
              <p className={`text-2xl font-bold ${getColorClass(m.color)}`}>
                {m.val ?? '—'}
              </p>
            </div>
          ))}
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl">
            <h3 className="flex items-center gap-2 mb-6 font-bold">
        ...
        <div className="mt-8 p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl flex items-center gap-2 text-emerald-500 text-sm">
          ...
          Systems Operational: All SEO modules syncing successfully
        </div>
